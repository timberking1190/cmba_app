...
## Member Cards — Phase 2: Apple + Google Wallet issuance (2026-08-04)

Wallet pass generation on top of the existing member-cards architecture (token.ts /
keys.ts / issuance.ts / verify.ts + the passes / apple-registrations / wallet-logs
collections). No parallel system introduced. The QR barcode on every wallet pass is the
same Ed25519 pass token the scanner already verifies, so wallet cards flow through
`/api/v1/member-cards/verify` unchanged.

### What shipped (code-complete)
- **Apple `.pkpass` generation + signing** — `src/lib/memberCards/applePass.ts`. Splits
  the password-protected `.p12` (node-forge) into signer cert + key, converts the WWDR
  G4 `.cer` (DER→PEM), and builds + signs the bundle with `passkit-generator` (manifest +
  detached PKCS#7 + zip). Node runtime only. Brand images embedded
  (`applePassAssets.ts`). Member download: `GET /api/v1/member-cards/apple/download`.
- **Apple PassKit web-service** — register / unregister / list-updatable / get-latest /
  log, under `/api/v1/member-cards/apple/v1/…` (the path Apple derives from the pass's
  `webServiceURL`). Per-pass auth via a **deterministic** `authenticationToken`
  (HMAC(secret, "applepass:<serial>")) — stable across re-downloads, never stored in
  plaintext. Backed by `appleWebService.ts` + the `apple-registrations` collection.
- **APNs push** — `appleApns.ts`. ES256 JWT (JOSE r||s signature), HTTP/2 to
  **api.push.apple.com** (production only — the provisioned key is topic-scoped +
  production), `apns-topic` = pass type id. Fired from a `passes.afterChange` hook on
  revoke/supersede/token-rotation (`hooks/walletPush.ts`); dead tokens (410) are pruned.
- **Google Wallet** — `googleWallet.ts`. Service-account access token (RS256 JWT bearer
  grant), idempotent Generic **class** + per-member **object** upsert (class defined in
  code, not the console), and a signed "Add to Google Wallet" save link with an
  inline-object fallback for a fresh/demo issuer. Member entry:
  `GET /api/v1/member-cards/google/save`. Gated by **demo mode** (default on).
- **Card UI** — `account/card/page.tsx` now shows "Add to Apple Wallet" / "Add to Google
  Wallet" for a scannable member with a live pass, only when that platform is configured;
  Google shows a preview/demo-mode caption.
- **Residency/privacy** — passes carry no credential detail (only member no. + name +
  role + season + the opaque token). Download routes are the member's own pass by user id
  (no client-supplied serial). All in-region (Vercel yul1 / ca-central-1).

### Automated gate (2026-08-04, worktree feat/wallet-phase2)
- `tsc --noEmit`: **clean (0 errors)**.
- `eslint --max-warnings=0`: **clean**.
- `vitest run`: **668/668 pass** across 74 files, incl. 29 new wallet tests. The Apple
  suite exercises the full build+sign chain against a node-forge fixture cert (real
  manifest + PKCS#7), the p12→PEM split, WWDR DER→PEM, ES256/RS256 JWT verification, the
  APNs 410-prune path, demo-mode defaulting, and the deterministic auth-token check.

### Set these in Vercel (values → var names) — never print secret contents
Non-secret identifiers are already defaulted in `.env.example`; set the secret blobs from
`~/Downloads/cmba-wallet-secrets` (base64-encode the binary files, e.g.
`base64 -i FILE | pbcopy`):

| Vercel env var | Source |
|---|---|
| `MEMBERCARD_APPLE_P12_BASE64` | base64 of `CMBA_PassTypeID.p12` |
| `MEMBERCARD_APPLE_P12_PASSWORD` | the .p12 export password |
| `MEMBERCARD_APPLE_WWDR_BASE64` | base64 of `AppleWWDRCAG4.cer` |
| `MEMBERCARD_APPLE_APNS_KEY_BASE64` | base64 of `AuthKey_VJQB268XAC.p8` |
| `MEMBERCARD_GOOGLE_SERVICE_ACCOUNT_KEY_BASE64` | base64 of `cmba-wallet-342aff67f342.json` |
| `MEMBERCARD_SIGNING_KID` / `_PRIVATE_KEY` / `_PUBLIC_KEYS` | the Ed25519 pass-token keypair (still required for any QR) |

Already defaulted (verify, don't re-enter): `MEMBERCARD_APPLE_TEAM_ID=D433C7C7BQ`,
`MEMBERCARD_APPLE_PASS_TYPE_ID=pass.ca.cmba.member`,
`MEMBERCARD_APPLE_APNS_KEY_ID=VJQB268XAC`, `MEMBERCARD_APPLE_APNS_ENVIRONMENT=production`,
`MEMBERCARD_GOOGLE_ISSUER_ID=3388000000023180302`, `MEMBERCARD_GOOGLE_DEMO_MODE=true`.

### Remaining verification — needs the real secrets / a device (operator)
1. **Apple:** set the env vars, then on a real iPhone tap "Add to Apple Wallet" from the
   card page → confirm it installs. Revoke/reissue the pass (rotate `currentJti`) →
   confirm the APNs push lands and Wallet refreshes.
2. **Google:** hit `/api/v1/member-cards/google/save` as a **registered test account** →
   confirm the save sheet opens and the card renders. Record the created `classId` into
   `MEMBERCARD_GOOGLE_CLASS_ID` (default `<issuerId>.cmba_member`).
3. **Scanner loop:** scan an installed wallet card at `/scan` → confirm the existing
   `/verify` returns a valid verdict (the barcode is the same signed token, so this is
   already covered by `smoke:member-cards`, but confirm once with a real wallet render).

### Two operator tasks that remain (not code)
1. Complete the Google business/payments profile in the Pay & Wallet Console.
2. Once a class exists and the profile is approved, request Google Wallet publishing
   access, then flip `MEMBERCARD_GOOGLE_DEMO_MODE=false`.

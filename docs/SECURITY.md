...
| MFA enforcement | Gated pages require enroll/challenge when MFA_ENFORCE on; no-op (zero change) when off | `src/lib/mfa/enforce.ts` (enforceMfa) wired into /account, /manage, /rep, /compliance/*; redirect targets unenforced (force-enrollment) | `enforce.test.ts`; live: flag-off behavior unchanged. GAP: Payload /admin panel slot still to add |

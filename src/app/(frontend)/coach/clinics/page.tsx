import { GraduationCap, ExternalLink, ShieldCheck, BookOpen, PlayCircle, ArrowRight, Calendar } from "lucide-react";
    ...
    <div>
      <section className="bg-hero-gradient border-b-2 border-cmba-red">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-10 lg:py-14">
          <div className="inline-flex items-center gap-2 bg-cmba-red/10 border border-cmba-red/30 px-3 py-1 mb-4">
            <GraduationCap size={14} className="text-cmba-red" />
            <span className="font-display font-bold text-xs text-cmba-red uppercase tracking-widest">Coach Development</span>
          </div>
          <h1 className="font-display font-black text-4xl lg:text-5xl text-white uppercase tracking-tight leading-[0.95]">
            TRAINING & <span className="text-cmba-red">CLINICS</span>
          </h1>
          <p className="text-cmba-grey mt-2 max-w-xl">
            CMBA&apos;s coach training runs online through reach360. Complete your required training, then keep building with the course library and athlete-development guides.
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-8 lg:py-12 space-y-12">
        ...
        <div>
          ...
          <p className="text-cmba-grey text-sm mb-6">Complete these before stepping on the floor.</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {required.map((r) => (
              <a key={r.title} href={r.href} target="_blank" rel="noopener noreferrer"
                className="bg-cmba-black/80 backdrop-blur-sm border border-white/12 hover:border-cmba-red/40 p-5 transition-colors group">
        ...
        <div>
          ...
          <p className="text-cmba-grey text-sm mb-6">Free CMBA courses hosted on reach360.</p>
          <div className="grid sm:grid-cols-2 gap-3">
            {courses.map((c) => (
              <a key={c.title} href={c.href} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-3 bg-cmba-black/80 backdrop-blur-sm border border-white/12 hover:border-cmba-red/40 p-4 transition-colors group">
        ...
        <div>
          ...
          <p className="text-cmba-grey text-sm mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {guides.map((g) => (
              <a key={g.age} href={g.href} target="_blank" rel="noopener noreferrer"
                className="bg-cmba-black/80 backdrop-blur-sm border border-white/12 hover:border-cmba-red/60 p-5 text-center transition-colors group">
        ...
        {/* Resources */}
        <div>
          <h2 className="font-display font-black text-2xl text-white uppercase tracking-tight mb-6">Coaching Resources</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {resources.map((r) => (
              <a key={r.title} href={r.href} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 bg-cmba-black/80 backdrop-blur-sm border border-white/12 hover:border-cmba-red/40 p-3 transition-colors group">

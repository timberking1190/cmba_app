...
export default async function ContestedPage() {
  ...
  const res = await payload.find({ collection: 'games', where: { status: { equals: 'contested' } }, sort: ['startAt', 'id'], depth: 1, limit: 100, overrideAccess: true })
  const games = (res.docs as unknown as Array<Record<string, unknown>>).map(toAdminGame)

  // Attach the latest open dispute reason per game.
  const ids = res.docs.map((g) => g.id)
  if (ids.length) {
    const disputes = await payload.find({ collection: 'disputes', where: { and: [{ game: { in: ids } }, { status: { equals: 'open' } }] }, depth: 0, limit: 200, overrideAccess: true })
    ...
    for (const g of games) g.disputeReason = reasonByGame.get(String(g.id))
  }

  // Also surface games awaiting a confirmation (reported but not final).
  const awaiting = await payload.find({ collection: 'games', where: { status: { equals: 'reported' } }, sort: ['startAt', 'id'], depth: 1, limit: 100, overrideAccess: true })
  const awaitingGames = (awaiting.docs as unknown as Array<Record<string, unknown>>).map(toAdminGame)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-8 lg:py-12 space-y-10">
      <div>
        ...
        <h1 className="font-display font-black text-3xl text-white uppercase tracking-tight mb-2">Contested <span className="text-cmba-red">queue</span></h1>
        <p className="text-cmba-grey text-sm mb-6 max-w-2xl">Games where a result was contested or a review was requested. Resolve each with a corrected final score, a forfeit, or a cancellation. A reason is required and recorded.</p>
      ...
      {awaitingGames.length > 0 && (
        <div>
          ...
          <SchedulingConsole games={awaitingGames} />
        </div>
      )}
    </div>
  )
